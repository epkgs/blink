...
func main() {
	...
	//在go中监听一个事件, 不带返回值
	//使用上下文获取参数
	app.IPC.Handle("go-event", func(arg1 string, arg2 string, arg3 int) {
		//参数是以js调用时传递的参数下标位置开始计算，从0开始表示第1个参数
		fmt.Printf("[go-event] arg1: '%s', arg2: '%s', arg3: %d\n", arg1, arg2, arg3)
	})

	//带有返回值的事件
	app.IPC.Handle("go-event-return", func(p1 string, p2 int, p3 bool, p4 float64, p5 string) string {
		fmt.Println("go-event-return event run")
	...
	// 使用形参接收参数
	// 在JS中入参类型必须相同
	app.IPC.Handle("go-event-argument", func(p1 int, p2 string, p3 float64, p4 bool, p5 string) {
	...
	app.IPC.Handle("go-event-argument-return", func(p1 int, p2 string, p3 float64, p4 bool, p5 string) string {
		fmt.Println("param1:", p1)
		fmt.Println("param2:", p2)
		fmt.Println("param3:", p3)
		fmt.Println("param4:", p4)
	...
	app.IPC.Handle("go-event-with-error", func() (interface{}, error) {
		return nil, errors.New("这是一个错误信息")
	})
